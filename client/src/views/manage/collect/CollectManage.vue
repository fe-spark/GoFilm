  ...
  <div class="container">
    ...
    >
      ...
      <el-table-column prop="syncPictures" align="center" label="同步图片">
        <template #default="scope">
          <el-switch
            ...
            :disabled="scope.row.grade == 1"
            v-model="scope.row.syncPictures"
            inline-prompt
            active-text="开启"
            inactive-text="关闭"
          />
      ...
      <el-table-column prop="grade" align="center" label="站点权重">
        <template #default="scope">
          <el-tag
            disable-transitions
            :type="`${scope.row.grade == 0 ? 'success' : 'info'}`"
          >
      ...
      <el-form :model="form.add">
        <el-form-item label="资源名称">
          <el-input
            ...
            placeholder="自定义资源名称(禁用汉字)"
          />
        </el-form-item>
        <el-form-item label="接口地址">
          <el-input
            v-model="form.add.uri"
            placeholder="资源采集链接,本站只采集综合资源或m3u8资源"
          />
        </el-form-item>
        <el-form-item label="间隔时长">
          <el-tooltip
            class="box-item"
            effect="dark"
            content="单次采集请求的时间间隔, 单位/ms"
            placement="top"
          >
            <el-input-number
              v-model="form.add.interval"
              :min="0"
              :step="100"
              step-strictly
            />
    ...
    <el-dialog v-model="dialogV.editV" title="修改资源站信息">
      <el-form :model="form.edit">
        ...
        <el-form-item label="图片同步">
          <el-switch
            v-model="form.edit.syncPictures"
            @change="restrict(1)"
            inline-prompt
            active-text="开启"
            inactive-text="关闭"
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button color="#cf48be" @click="apiTest(form.edit)"
            >测试</el-button
          >
          <el-button color="#9b49e7" @click="updateSite(form.edit)"
            >更新</el-button
          >
    ...
    <el-dialog v-model="dialogV.batchV" width="450px" title="多资源站一键采集">
      <el-form :model="form.batch">
        ...
        <el-form-item label="采集时长">
          <el-tooltip
            class="box-item"
            effect="dark"
            content="采集最近x小时更新的影片,负数则默认采集所有资源"
            placement="top"
          >
            <el-input-number
              v-model="form.batch.time"
              :step="1"
              step-strictly
            />
          </el-tooltip>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button color="#9b49e7" @click="startBatchCollect"
            >确认执行</el-button
          >
...
const startBatchCollect = () => {
  ApiPost(`/manage/spider/start`, {
    ids: form.batch.ids,
    time: form.batch.time,
    batch: true,
  }).then((resp: any) => {
    if (resp.code === 0) {
...
// 获取当前正在采集的状态
const getCollectingState = () => {
  ApiGet(`/manage/collect/collecting/state`, {}, false).then((resp: any) => {
    if (resp.code === 0) {
      data.activeCollectIds = resp.data || [];
    }
  });
};

// 停止采集任务
const stopCollectTask = (id: string) => {
  ApiGet(`/manage/collect/stop`, { id: id }).then((resp: any) => {
    if (resp.code === 0) {
      ElMessage.success({ message: resp.msg });
      getCollectingState();
    } else {
...
const clearFilms = () => {
  ...
  ApiGet(`/manage/spider/clear`, { password: data.password }).then(
    (resp: any) => {
      if (resp.code === 0) {
        ElMessage.success({ message: resp.msg });
      } else {
...
const reCollectFilm = () => {
  ...
  ApiGet(`/manage/spider/zero`, { password: data.password }).then(
    (resp: any) => {
      if (resp.code === 0) {
        ElMessage.success({ message: resp.msg });
      } else {
